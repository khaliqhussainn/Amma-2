export const newsArticles = [
  {
    id: 2,
    slug: 'amma-president-meets-senator-van-hollen',
    title: 'Advocacy in Action: AMMA President Meets Senator Chris Van Hollen',
    excerpt: 'Dr. Mohsin Ansari, AMMA President and ICNA Vice President of Governmental Affairs, met with Senator Chris Van Hollen to present a recognition award and raise urgent humanitarian concerns, including the crisis in Gaza and the case of Dr. Hussam Abu Safiya.',
    category: 'Humanitarian',
    date: 'August 20, 2026',
    readTime: '4 min read',
    author: 'AMMA Communications Team',
    authorTitle: 'AMMA National',
    image: '/news/amma-van-hollen-award.jpg',
    featured: false,
    content: [
      {
        type: 'intro',
        text: "Alhamdulillah. Dr. Mohsin Ansari, President of the American Muslim Medical Association and Vice President of Governmental Affairs at the Islamic Circle of North America (ICNA), met with Senator Van Hollen to discuss pressing humanitarian and healthcare concerns affecting the American Muslim community and people around the world. He also presented a recognition award on behalf of ICNA."
      },
      {
        type: 'paragraph',
        text: "The award recognizes Senator Van Hollen's courageous leadership, commitment to justice, and continued support for issues of importance to the American Muslim community. Dr. Ansari also discussed ICNA's contributions to American society and its continued efforts in serving communities across the country."
      },
      {
        type: 'image',
        src: '/news/amma-van-hollen-group.jpg',
        alt: 'Dr. Mohsin Ansari and community leaders present a recognition award to Senator Chris Van Hollen',
        caption: 'Dr. Mohsin Ansari and fellow community leaders present Senator Chris Van Hollen with an ICNA Congressional Leadership Award.'
      },
      {
        type: 'heading',
        text: 'Addressing the Humanitarian Crisis in Gaza'
      },
      {
        type: 'paragraph',
        text: 'A significant part of the discussion focused on the ongoing humanitarian crisis in Gaza and the devastating impact on its healthcare system and medical professionals.'
      },
      {
        type: 'paragraph',
        text: 'Dr. Ansari highlighted the severe challenges facing healthcare workers and the urgent need to protect medical personnel. He urged Senator Van Hollen to continue his leadership and advocacy on behalf of the people of Gaza and to support efforts toward humanitarian relief, justice, and peace.'
      },
      {
        type: 'paragraph',
        text: 'As an organization representing Muslim healthcare professionals, AMMA remains deeply concerned about the circumstances faced by healthcare workers during the ongoing crisis and believes that medical professionals must be protected and allowed to fulfill their essential role of caring for patients.'
      },
      {
        type: 'heading',
        text: 'Advocacy for Dr. Hussam Abu Safiya'
      },
      {
        type: 'paragraph',
        text: "Another urgent issue raised during the meeting was the case of Dr. Hussam Abu Safiya, whose continued detention and reported continued deterioration in health remain deeply concerning."
      },
      {
        type: 'paragraph',
        text: "Dr. Ansari urged Senator Van Hollen to intensify efforts to help secure Dr. Abu Safiya's immediate release. His case also reflects broader concerns regarding the safety and treatment of healthcare professionals affected by the conflict."
      },
      {
        type: 'paragraph',
        text: 'By bringing these concerns directly to policymakers, AMMA continues to advocate for the protection of healthcare workers and the preservation of humanitarian principles.'
      },
      {
        type: 'heading',
        text: 'A Commitment to Continued Engagement'
      },
      {
        type: 'paragraph',
        text: 'We are grateful to Senator Van Hollen for his accessibility, sincere engagement, and willingness to listen to the concerns raised during the meeting.'
      },
      {
        type: 'paragraph',
        text: 'The meeting reflects the importance of continued engagement between healthcare professionals, community representatives, and policymakers in addressing humanitarian concerns and advocating for justice and peace.'
      },
      {
        type: 'paragraph',
        text: 'AMMA remains committed to advocating for healthcare workers, supporting humanitarian causes, and engaging constructively with policymakers on issues affecting the dignity, safety, and well-being of communities.'
      },
      {
        type: 'quote',
        text: 'May Allah (SWT) guide us all to continue striving for justice, compassion, and peace for all.'
      }
    ]
  },
  {
    id: 1,
    slug: 'amma-launches-icna-2026',
    title: 'AMMA Launches at ICNA Convention 2026',
    excerpt: 'Launching at one of the largest gatherings of Muslims in North America gave AMMA the opportunity to connect directly with the people it was created to serve. Hundreds of healthcare professionals joined on the spot.',
    category: 'Announcements',
    date: 'May 23–25, 2026',
    readTime: '3 min read',
    author: 'AMMA Communications Team',
    authorTitle: 'AMMA National',
    image: '/news/amma-icna-2026-launch.jpeg',
    featured: true,
    content: [
      {
        type: 'intro',
        text: "Launching at one of the largest gatherings of Muslims in North America gave AMMA the opportunity to connect directly with the people it was created to serve. Throughout the convention, Muslim physicians, dentists, nurses, pharmacists, therapists, medical students, and other healthcare professionals visited the AMMA booth to learn more about the association, meet the team, and discover its vision for building a stronger professional community."
      },
      {
        type: 'heading',
        text: 'A Warm Welcome from the Community'
      },
      {
        type: 'paragraph',
        text: 'The response throughout the convention was encouraging.'
      },
      {
        type: 'paragraph',
        text: 'Over the course of the event, hundreds of healthcare professionals stopped by the booth to ask questions, learn about membership, and connect with fellow clinicians. Many chose to join AMMA during the convention, reflecting a shared interest in building a professional network that supports Muslim healthcare professionals through every stage of their careers.'
      },
      {
        type: 'paragraph',
        text: 'Beyond the memberships, what stood out most were the conversations. Visitors shared their experiences, discussed the challenges they face in healthcare, and expressed the importance of having an organization that brings Muslim professionals together with a shared purpose.'
      },
      {
        type: 'heading',
        text: 'Building a Community Around Shared Values'
      },
      {
        type: 'paragraph',
        text: "For many attendees, this was their first introduction to AMMA's mission."
      },
      {
        type: 'paragraph',
        text: 'AMMA exists to unite and empower Muslim healthcare professionals through professional development, mentorship, advocacy, and service. Every initiative is guided by four core values that define the association\'s purpose.'
      },
      {
        type: 'quote',
        text: "To unite and empower Muslim healthcare professionals through professional development, spiritual growth, mentorship, advocacy, collaboration, and service to humanity, guided by the teachings of Islam and dedicated to seeking the pleasure of Allah.",
        author: 'AMMA Mission Statement'
      },
      {
        type: 'list',
        items: [
          'Ihsan (Excellence): Striving for the highest standards in clinical care, professionalism, and personal growth.',
          'Rahma (Compassion): Serving patients and communities with empathy, kindness, and dignity.',
          'Unity: Creating meaningful connections among healthcare professionals across specialties and career stages.',
          'Service: Giving back through community health initiatives, humanitarian work, and advocacy that benefits society.'
        ]
      },
      {
        type: 'paragraph',
        text: 'These values shaped every interaction at the booth and continue to guide AMMA as it grows.'
      },
      {
        type: 'heading',
        text: 'Looking Ahead'
      },
      {
        type: 'paragraph',
        text: 'Launching at ICNA Convention 2026 was an important first step, but it is only the beginning.'
      },
      {
        type: 'paragraph',
        text: 'With hundreds of new members joining during the convention, AMMA is now focused on expanding its chapter network, creating opportunities for professional development, strengthening mentorship programs, and supporting healthcare professionals through advocacy and service.'
      },
      {
        type: 'paragraph',
        text: 'To everyone who visited the booth, became a member, or took the time to learn about AMMA, thank you for being part of this milestone.'
      },
      {
        type: 'paragraph',
        text: 'If you were not able to connect with us at the convention, you can still become a member and be part of a growing community dedicated to excellence, compassion, unity, and service.'
      }
    ]
  }
];

export const categories = ["All", "Announcements", "Humanitarian", "Clinical Ethics", "Chapters"];
